...
class GraphPyramid():
    ...
    def make_graph(self, heart_name, K=6):
        ...
        h_P1, h_g1 = self.get_embeddings(h_g, heart_name, 1)
        h_gs.append(h_g1)
        h_Ps.append(h_P1)

        h_P2, h_g2 = self.get_embeddings(h_g1, heart_name, 2)
        h_gs.append(h_g2)
        h_Ps.append(h_P2)
        
        h_P3, h_g3 = self.get_embeddings(h_g2, heart_name, 3)
        h_gs.append(h_g3)
        h_Ps.append(h_P3)
        
        h_P4, h_g4 = self.get_embeddings(h_g3, heart_name, 4)
        h_gs.append(h_g4)
        h_Ps.append(h_P4)

        t_gs, t_Ps = [], []
        matFiles = scipy.io.loadmat(osp.join(self.path_structure, '{}_t0.mat'.format(heart_name)), squeeze_me=True, struct_as_record=False)
        face = matFiles['face']
        edge_index = self.face2edge(face)
        ...
        t_P1, t_g1 = self.get_embeddings(t_g, heart_name, 1, is_heart=False)
        t_gs.append(t_g1)
        t_Ps.append(t_P1)
        
        t_P2, t_g2 = self.get_embeddings(t_g1, heart_name, 2, is_heart=False)
        t_gs.append(t_g2)
        t_Ps.append(t_P2)
        
        t_P3, t_g3 = self.get_embeddings(t_g2, heart_name, 3, is_heart=False)
        t_gs.append(t_g3)
        t_Ps.append(t_P3)
        ...
        if self.method == 'graclus_hier':
            ...
            matFiles = scipy.io.loadmat(osp.join(self.path_structure, '{}_center.mat'.format(heart_name)), squeeze_me=True, struct_as_record=False)
            center = matFiles['center']
            center = torch.Tensor(center).view(1, -1)
            fake_data = testdata.x[0, :].view(1, -1, self.seq_len)
            center_data = Data(x=fake_data, y=testdata.y[0], pos=center)
            
            Hs, Ps = [], []
            H_enc, P_enc = self.bipartite_graph(center_data, t_g3)
            Hs.append(H_enc)
            Ps.append(P_enc)
