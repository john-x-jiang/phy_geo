...
class GraphPyramid():
    ...
    def make_graph(self, heart_name, K=6):
        ...
        matFiles = scipy.io.loadmat(osp.join(self.path_structure, '{}_t1.mat'.format(heart_name)), squeeze_me=True, struct_as_record=False)
        face = matFiles['face']
        cor = matFiles['cor']
        cluster = matFiles['cluster']
        edge_index = self.face2edge(face)
        t_P1, t_g1 = copy.deepcopy(self.cluster_mesh(t_g, cluster, cor, edge_index))
        # self.save_connection(t_g1, name='t1')
        
        matFiles = scipy.io.loadmat(osp.join(self.path_structure, '{}_t2.mat'.format(heart_name)), squeeze_me=True, struct_as_record=False)
        face = matFiles['face']
        cor = matFiles['cor']
        cluster = matFiles['cluster']
        edge_index = self.face2edge(face)
        t_P2, t_g2 = copy.deepcopy(self.cluster_mesh(t_g1, cluster, cor, edge_index))
        # self.save_connection(t_g2, name='t2')
        
        matFiles = scipy.io.loadmat(osp.join(self.path_structure, '{}_t3.mat'.format(heart_name)), squeeze_me=True, struct_as_record=False)
        face = matFiles['face']
        cor = matFiles['cor']
        cluster = matFiles['cluster']
        edge_index = self.face2edge(face)
        t_P3, t_g3 = copy.deepcopy(self.cluster_mesh(t_g2, cluster, cor, edge_index))
