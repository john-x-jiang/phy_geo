...
def evaluation(model, x_loader, model_dir, corMfree, heart_name):
    ...
    scipy.io.savemat(model_dir + '/data/Tmp_{}.mat'.format(heart_name),
                     {'U': all_recons, 'Y': all_ys, 'label': all_label})
...
def train(epoch, model, loss_function, phy_mode, smooth, optimizer, train_loaders, batch_size, seq_len, model_dir,
          anneal, sample=1):
    ...
    for heart_name in heart_names:
        ...
        for idx, data in enumerate(train_loader):
            ...
            if type(rtn) == tuple:
                loss, bce, kld = loss_function(mu_theta, logvar_theta, x, mu, logvar, batch_size, seq_len, epoch,
                                               anneal)
            else:
                loss, bce, kld, phy, smt = loss_function(mu_theta, x, y_, y, l_t, l_h, mu, logvar, phy_mode, smooth,
                                                         batch_size, seq_len, epoch, anneal)
            loss.backward()
            ...
            n += 1
            utils.inline_print(
                f'Running epoch {epoch}, heart {heart_name}, batch {n}, Average loss for epoch: {str(train_loss / (n * batch_size))}')
...
def train_vae(model, optimizer, train_loaders, test_loaders, loss_function, phy_mode, smooth,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        train_acc, bce_acc_t, kld_acc_t, phy_acc_t, smt_acc_t = \
            train(epoch, model, loss_function, phy_mode, smooth, optimizer, train_loaders, batch_size, seq_len,
                  model_dir, anneal, sample)
        ...
        logs = 'Epoch: {:03d}, Time: {:.4f}, Train: {:.4f}, Test: {:.4f}'.format(epoch, (te - ts) / 60, train_acc,
                                                                                 test_acc)
        print(logs)
